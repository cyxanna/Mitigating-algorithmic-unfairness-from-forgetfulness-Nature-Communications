...
def compute_equalized_odds_loss(logits, labels, protected_attr, method="pairwise_squared_diff"):
    """
    ...
    for epoch in range(config.unlearning_epochs):
        ...
        for batch_idx, (X_remain, y_remain, protected_attr) in enumerate(remaining_loader):
            ...
            fair_loss, fair_stats = compute_equalized_odds_loss(
                ...
                y_remain, 
                protected_attr,
                method=getattr(config, 'fairness_loss_type', 'pairwise_squared_diff'),
            )
            ...
            grad_forget_processed = gram_schmidt_process(
                ...
                method=getattr(config, 'gram_schmidt_type', 'orthonormal_basis'),
                normalize=getattr(config, 'normalize_forget_grad', False),
            )
